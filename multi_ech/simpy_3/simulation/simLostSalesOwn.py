...
class stocking_facility(object):
    ...
    def check_inventory(self):
        while True:
            ...
            if (
                self.inventory_position <= 1.05 * self.ROP
            ):  # add 5% to avoid rounding issues
    ...
    def serve_customer(self):
        while True:
            ...
            yield self.env.timeout(1.0)
            dist = getattr(st, self.distName)
            dist = dist(*self.distParams)
            demand = dist.rvs()
...
def simulate_network(
    ...
    for i in range(num_nodes):
        if (
            i == 0
        ):  # then it is the first supply node, which is assumed to have infinite inventory
            s = stocking_facility(
                env,
                i,
                1,
