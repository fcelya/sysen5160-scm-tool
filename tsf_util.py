...
def pareto(df, show=False):
    ...
    pareto = {}
    for p in df_agg.index:
        pareto[p] = (df_agg["count"][p], df_agg["cumperc"][p])

    if show:
        fig, ax = plt.subplots()
        ax.bar(list(range(len(df_agg.index))), df_agg["count"])
        ax2 = ax.twinx()
        ax2.plot(list(range(len(df_agg.index))), df_agg["cumperc"], color="red")
        plt.show()
    return pareto
